		...
		if (err) {
			...
			if (!user) {
				...
				bcrypt.compare(password, user.password, function(err, check){
					if (check) {
						// devolver los datos del usuario loggueado
						if (params.gethash) {
							// devolver token jwt
							res.status(200).send({
...
function updateUser(req, res){
	...
	if (userId != req.user.sub) {
        return res.status(500).send({message: 'No tienes permiso de actualizar este permiso'});
	}

	User.findByIdAndUpdate(userId, params, function(err, userUpdated){
		if (err) {
			res.status(500).send({message: 'Error a actualizar el usuario'});
		}else{
			if (!userUpdated) {
				res.status(404).send({
